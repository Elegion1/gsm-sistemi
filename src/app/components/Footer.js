import Image from "next/image";
import owner from "@/data/owner.json";
import links from "@/data/links.json";

export default function Footer() {
  return (
    <footer className="bg-b vw-100 px-5 pt-5 pb-3 footer">
      <div className="row">
        <div className="col-md-4 col-12 d-flex justify-content-between flex-column mb-4">
          <Image
            src="/images/logo_long.svg"
            height={75}
            width={366}
            alt="Logo-gsm-sistemi"
            loading="eager"
          />
          <p className="text-d">Qualita, design e sicurezza per la tua casa</p>
        </div>
        <div className="col-md-4 col-12 d-flex justify-content-center align-items-center flex-column">
          <p className="text-uppercase text-d m-0">link utili</p>
          {links.map((link, index) => (
            <a
              key={index}
              className="text-d text-uppercase text-reset text-decoration-none"
              href={`${link.href}`}
            >
              {link.label}
            </a>
          ))}
        </div>
        <div className="col-md-4 col-12 text-d text-end">
          <p className="m-0">{owner.legalName}</p>
          <p className="m-0">
            {owner.address.street}, {owner.address.streetNumber} <br />
            {owner.address.city}, {owner.address.locality} {owner.address.zip}{" "}
            {owner.address.province}
          </p>
          <p className="m-0 text-uppercase">p.iva: {owner.vatNumber}</p>
          <p className="m-0 text-uppercase">sdi: {owner.sdiCode}</p>
          <p className="m-0 text-uppercase">tel: {owner.phone}</p>
          <p className="m-0 text-uppercase">
            pec: <span className="text-lowercase">{owner.pec}</span>
          </p>
          <p className="m-0 text-uppercase">
            email: <span className="text-lowercase">{owner.email}</span>
          </p>
        </div>
      </div>
      <div className="container mt-5">
        <p className="text-d text-center m-0">
          © {new Date().getFullYear()} {owner.companyName} All rights reserved.
        </p>
        <p className="text-d text-center m-0">
          Sito realizzato e ottimizzato da{" "}
          <a
            className="text-reset text-decoration-none"
            href="https://www.elegion.it"
          >
            Giovanni Sugamiele
          </a>
        </p>
      </div>
    </footer>
  );
}
